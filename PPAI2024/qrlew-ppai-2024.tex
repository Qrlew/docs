...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/Qrlew - process.pdf} % Reduce the figure size so that it is slightly narrower than the column.
...
\begin{description}
    ...
    \item[A \texttt{Join}] This combines two input \texttt{Relation}s as a \texttt{SELECT * FROM left JOIN right ON expr} would do it. The provacy properties are more complex to propagate in this case.
\end{description}
