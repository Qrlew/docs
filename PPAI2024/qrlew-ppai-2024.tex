...
Let the column be a vector of $N$ real numbers: $x = \left(x_1,\ldots, x_N\right)\in\mathbb{R}^N$. We note: $\pi(x_k) = i \in \{1,\ldots,n\}$ the PEID and $g(x_k) = j \in \{1,\ldots,m\}$ the grouping key associated to $x_k$. 
We want to compute all the:
$$S_j = \sum_{g(x_k) = j} x_k$$
...
\begin{enumerate}
    ...
    $$s_{i,j} = \sum_{\substack{\pi(x_k) = i\\g(x_k) = j}}x_k$$
    The $s_i$'s $\ell^2$ norms are then clipped:
