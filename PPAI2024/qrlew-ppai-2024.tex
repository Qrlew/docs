...
Even if some bounds ar known for some \texttt{Relation}s like source \texttt{Table}s, it is far from obvious to propagate these bounds through the steps of the computation.

To help with range propagation, \qrlew{} introduces two simple concepts.
\begin{itemize}
    \item The \emph{$k$-Intervals}, which are finite union of at most $k$ closed intervals. A \emph{$k$-Intervals} can be noted:
    $$I = \bigcup_{i=1}^{j\leq k}\left[a_i, b_i\right]$$
    Note that the union of \emph{$k$-Intervals} may not be a \emph{$k$-Intervals} as it may be the union of more than $k$ intervals.
    ...
    \item The \emph{piecewise-coordinatewise-monotonic-functions} or \emph{piecewise-monotonic-functions}, which are functions $f: \mathbb{R}^n \rightarrow \mathbb{R}$ whose domain can be partitioned in product of intervals on which they are \emph{coordinatewise-monotonic}.
    The image of a product of \emph{$k$-Intervals} by a \emph{piecewise-monotonic-function} can be computed as a \emph{$k^n$-Intervals} 
    ...
    One can show that: $$f(I) = \bigcup_{\substack{1\leq i_1\leq k\\\ldots\\1\leq i_n\leq k}}\text{Conv}\left(f\left( \left\{a_{i_1}, b_{i_1}\right\}\times \ldots \times \left\{a_{i_n}, b_{i_n}\right\}\right)\right)$$
\end{itemize}
