...
With \qrlew{}, a \emph{data practitionner} can express his data queries in standard SQL; the \emph{data owner} can run the rewritten query without any technical integration and with strong privacy garantees on the output; and the query rewriting can be operated by a privacy-expert that has to be trusted by the owner, but may belong to a separate organizations.
\end{abstract}
...
In this paper, \emph{datasets} refers to a collection of elements in some domain $\mathcal{X}$, labelled by an identifier $i\in \mathcal{I}$ identifying the entity whose privacy we want to protect. This entity will be called \emph{Protected Entity} (PE) and the identifier will be referd to as \emph{Protected Entity ID} (PEID). Let $\mathcal{D}$ be the set of datasets of arbitrary size with a protected entity.
...
Datasets $D, D' \in \mathcal{D}$ are adjacent if they are equal up to the addition or removal of all entries sharing the same PEID. Note that this is a slightly restricted definition of adjacency, suited to our practical needs. It is close to that of \cite{liu2020learning, wilson2019differentially} where a user can have many samples.
...
\begin{description}
    ...
    \item[A \texttt{Reduce}] A \texttt{Reduce} takes an input \texttt{Relation} and aggregates some columns, possibly group by group. It acts as a \texttt{SELECT aggregates FROM input GROUP BY expr}. This is where the rewriting into DP will happen as described in section~\ref{sec:rewritting}.
    \item[A \texttt{Join}] This \texttt{Relation} combines two input \texttt{Relation}s as a \texttt{SELECT * FROM left JOIN right ON expr} would do it. The privacy properties are more complex to propagate in this case.
\end{description}
It may also be a static list of values or a set operation between two \texttt{Relation}s, but those are less important for our uses.
...
\begin{itemize}
    ...
    \item The concept of \emph{piecewise-coordinatewise-monotonic-functions} or \emph{piecewise-monotonic-functions}, which are functions $f: \mathbb{R}^n \rightarrow \mathbb{R}$ whose domain can be partitioned in products of intervals on which they are \emph{coordinatewise-monotonic}.
    The image of a product of $n$ \emph{$k$-Interval}s by a \emph{coordinatewise-monotonic-function} can be easily computed as a \emph{$k$-Interval}.
    Indeed, let $I$ be:
    ...
    If $f$ is \emph{piecewise-monotonic} then one can show that on each partition $P_j$:
    $$f(I \cap P_j) = \bigcup_{\substack{1\leq i_1\leq k\\\ldots\\1\leq i_n\leq k}}\text{Conv}\left(f\left( \left\{a_{i_1}, b_{i_1}\right\}\times \ldots \times \left\{a_{i_n}, b_{i_n}\right\}\right)\right)$$
    where $\text{Conv}\left(f\left( \left\{a_{i_1}, b_{i_1}\right\}\times \ldots \times \left\{a_{i_n}, b_{i_n}\right\}\right)\right)$ is efficiently computed in $n$ steps, thanks to the coordinatewise monotony of $f$ on $P_j$.
\end{itemize}
...
Tables in a database rarely come properly formated for privacy-preserving applications. Many rows in many tables may refer to one individual, hence, \emph{adding or removing an individual} means 
